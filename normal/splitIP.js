...
function restoreIPAdress(str) {
    const res = [];
    function dfs(resArr, start) {
        if (resArr.length == 4 && start == str.length) {
            res.push(resArr.join('.'));
            return;
        }
        if (resArr.length == 4 && start < str.length) return;
        for (let i = 1; i <= 3; ++i) {
            if (start + i - 1 >= str.length) return;
            if (i > 1 && str.startsWith('0')) return;
            const s = str.substring(start, start + i)
            if (s.length == 3 && Number(s) > 255) return;
            resArr.push(s);
            dfs(resArr, start + i);
            resArr.pop();
        }
...
function splitIpAdress(str) {
    ...
    function dfs(resArr, start) {
        ...
        for (let len = 1; len <= 3; ++len) {
            if (start + len - 1 > str.length) return;
            if (len > 1 && str.startsWith('0')) return;
            const s = str.substring(start, start + len);
            if (len == 3 && Number(s) > 255) return;
            resArr.push(s);
            dfs(resArr, start + len);
            resArr.pop();
        }
    }
    dfs([], 0);
...
// console.log(restoreIPAdress(str));
console.log(splitIpAdress(str));
